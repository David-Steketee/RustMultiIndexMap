...
#[proc_macro_derive(HelloMacro, attributes(multiindex_ref))]
...
fn impl_hello_macro(ast: &syn::DeriveInput) -> TokenStream {
    ...
    let index_variables = fields.iter().filter_map(|f| {
        for attr in &f.attrs {
            if attr.path.segments.len() == 1 && attr.path.segments[0].ident == "multiindex_ref" {
    ...
    let methods = index_variables.clone().map(|(var_name, var_type)| {
        ...
        quote! {
            ...
            pub fn #getter_func_ident(&self, key: #var_type) -> std::option::Option<&#object_name> {
                let idxOpt = self.#hashmap_ident.get(key);
                if let Some(idx) = idxOpt {
                    return Some(self.slab[idx]);
                } else {
                    return None;
                }
            }
            ...
            //remove
            pub fn #remove_func_ident(&self, key: #var_type) -> Result<#object_name, #var_type> {
               let idxOpt = self.#hashmap_ident.get(key);
               if let Some(idx) = idxOpt {
                   let obj = &self.slab.remove(idx);
    ...
    let check_indexes_unused = index_variables.clone().map(|(var_name, _var_type)| {
        ...
        quote! {
            if let Entry::Occupied(_existing) = self.#hashmap_ident.entry(obj.#var_name) {
    ...
    let add_slab_index_to_maps = index_variables.clone().map(|(var_name, _var_type)| {
        let hashmap_ident = var_to_map(var_name);
        quote! {
            self.#hashmap_ident.insert(obj.#var_name, new_entry.key());
        }
    });
...
fn var_to_map(var: &syn::Ident) -> syn::Ident {
    let hashmap_name = format!("{}Map", var);
